...
describe('extractGlobalFlags env var fallbacks', () => {
  test('LAUREL_QUIET enables quiet mode when CLI did not set it', () => {
    const { flags } = extractGlobalFlags(['build'], { LAUREL_QUIET: '1' });
    expect(flags.quiet).toBe(true);
  });

  test('CLI --quiet takes priority over LAUREL_QUIET=false', () => {
    const { flags } = extractGlobalFlags(['--quiet', 'build'], { LAUREL_QUIET: 'false' });
    expect(flags.quiet).toBe(true);
  });

  test('LAUREL_QUIET=false leaves quiet false', () => {
    const { flags } = extractGlobalFlags(['build'], { LAUREL_QUIET: 'false' });
    expect(flags.quiet).toBe(false);
  });

  test('throws on unparseable LAUREL_QUIET', () => {
    expect(() => extractGlobalFlags(['build'], { LAUREL_QUIET: 'maybe' })).toThrow();
  });

  test('LAUREL_VERBOSE=2 sets verboseCount=2', () => {
    const { flags } = extractGlobalFlags(['build'], { LAUREL_VERBOSE: '2' });
    expect(flags.verboseCount).toBe(2);
  });

  test('CLI -V overrides LAUREL_VERBOSE', () => {
    const { flags } = extractGlobalFlags(['-V', 'build'], { LAUREL_VERBOSE: '3' });
    expect(flags.verboseCount).toBe(1);
  });

  test('LAUREL_VERBOSE=0 is a no-op', () => {
    const { flags } = extractGlobalFlags(['build'], { LAUREL_VERBOSE: '0' });
    expect(flags.verboseCount).toBe(0);
  });

  test('throws on non-integer LAUREL_VERBOSE', () => {
    expect(() => extractGlobalFlags(['build'], { LAUREL_VERBOSE: 'foo' })).toThrow(
      /LAUREL_VERBOSE/,
    );
  });

  test('throws on negative LAUREL_VERBOSE', () => {
    expect(() => extractGlobalFlags(['build'], { LAUREL_VERBOSE: '-1' })).toThrow();
  });
  ...
  test('LAUREL_NO_COLOR=0 re-enables color even if NO_COLOR=1', () => {
    const { flags } = extractGlobalFlags(['build'], { NO_COLOR: '1', LAUREL_NO_COLOR: '0' });
    expect(flags.noColor).toBe(false);
  });
  ...
  test('LAUREL_JSON=1 sets json mode', () => {
    const { flags } = extractGlobalFlags(['build'], { LAUREL_JSON: '1' });
    expect(flags.json).toBe(true);
    expect(flags.logFormat).toBe('json');
  });

  test('LAUREL_LOG_FORMAT sets logger format without enabling command json', () => {
    const { flags } = extractGlobalFlags(['build'], { LAUREL_LOG_FORMAT: 'json' });
    expect(flags.json).toBe(false);
    expect(flags.logFormat).toBe('json');
  });

  test('CLI --log-format overrides LAUREL_JSON logger mode while keeping command json', () => {
    const { flags } = extractGlobalFlags(['--log-format=pretty', 'build'], { LAUREL_JSON: '1' });
    expect(flags.json).toBe(true);
    expect(flags.logFormat).toBe('pretty');
  });

  test('throws on invalid LAUREL_LOG_FORMAT', () => {
    expect(() => extractGlobalFlags(['build'], { LAUREL_LOG_FORMAT: 'compact' })).toThrow(
      /LAUREL_LOG_FORMAT/,
    );
  });

  test('LAUREL_DEBUG=true sets debug mode', () => {
    const { flags } = extractGlobalFlags(['build'], { LAUREL_DEBUG: 'true' });
    expect(flags.debug).toBe(true);
  });

  test('LAUREL_WARNINGS_AS_ERRORS=true enables warnings-as-errors mode', () => {
    const { flags } = extractGlobalFlags(['build'], { LAUREL_WARNINGS_AS_ERRORS: 'true' });
    expect(flags.warningsAsErrors).toBe(true);
  });

  test('project .laurelrc supplies global defaults below env and CLI flags', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'laurel-rc-global-'));
    try {
      await writeFile(
      ...
      const fromEnv = extractGlobalFlags(['build'], { LAUREL_VERBOSE: '1' }, dir).flags;
      expect(fromEnv.verboseCount).toBe(1);

      const fromCli = extractGlobalFlags(['-V', 'build'], { LAUREL_VERBOSE: '3' }, dir).flags;
  ...
  test('user global config supplies defaults below project rc, env, and CLI flags', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'laurel-rc-global-project-'));
    const xdg = await mkdtemp(join(tmpdir(), 'laurel-rc-global-user-'));
    try {
      ...
      await writeFile(join(dir, '.laurelrc.json'), JSON.stringify({ global: { verbose: 2 } }));
      ...
      const fromEnv = extractGlobalFlags(
        ['build'],
        { XDG_CONFIG_HOME: xdg, LAUREL_LOCALE: 'en-US' },
        dir,
      ).flags;
