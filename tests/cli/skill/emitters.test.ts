...
describe('claude emitter', () => {
  let cwd: string;
  beforeEach(async () => {
    cwd = await mkdtemp(join(tmpdir(), 'laurel-skill-claude-'));
  });
  afterEach(async () => {
  ...
  test('writes SKILL.md with Claude-compatible {name, description} frontmatter', async () => {
    ...
    // Should not surface Laurel-internal frontmatter keys.
    expect(md).not.toContain('version:');
    expect(md).not.toContain('applies_to:');
  });

  test('writes a .laurel.json receipt for version tracking', async () => {
    const skill = fixtureSkill();
    const target = await installSkillForClaude(cwd, skill);
    const receipt = JSON.parse(
      await readFile(join(target, '.laurel.json'), 'utf8'),
...
describe('codex emitter', () => {
  let cwd: string;
  beforeEach(async () => {
    cwd = await mkdtemp(join(tmpdir(), 'laurel-skill-codex-'));
  });
  afterEach(async () => {
    await rm(cwd, { recursive: true, force: true });
  });
