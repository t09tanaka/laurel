...
// operator enables `[components.portal]`, Nectar's render path must keep that
...
describe('example build with portal enabled (#494)', () => {
  test('Source navigation portal markup survives a default nectar build', async () => {
    const exampleDir = join(process.cwd(), 'example');
    const cwd = await mkdtemp(join(tmpdir(), 'nectar-example-portal-'));
    ...
    const originalConfig = await readFile(join(exampleDir, 'nectar.toml'), 'utf8');
    await writeFile(
  ...
  test('external provider rewrites portal buttons to the provider URL', async () => {
    const exampleDir = join(process.cwd(), 'example');
    const cwd = await mkdtemp(join(tmpdir(), 'nectar-example-portal-rewrite-'));
    await cp(exampleDir, cwd, { recursive: true });

    const originalConfig = await readFile(join(exampleDir, 'nectar.toml'), 'utf8');
    await writeFile(
      join(cwd, 'nectar.toml'),
      `${originalConfig}\n[components.portal]\nprovider = "buttondown"\npublication = "nectar-demo"\n`,
      'utf8',
    );
    ...
    expect(indexHtml).toMatch(/href="https:\/\/buttondown\.email\/nectar-demo"/);
