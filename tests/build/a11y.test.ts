...
describe('injectSkipLink', () => {
  test('inserts a skip link as the first node inside <body>', () => {
    ...
    const skipPos = out.indexOf('class="laurel-skip-link');
  ...
  test('emits an anchor targeting #main with visible text', () => {
    ...
    expect(out).toMatch(/<a [^>]*class="laurel-skip-link[^"]*"[^>]*href="#main"[^>]*>/);
  ...
  test('targets the existing first <main> id when a theme uses a non-main id', () => {
    ...
    expect(out).toMatch(/<a [^>]*class="laurel-skip-link[^"]*"[^>]*href="#site-main"[^>]*>/);
  });
  ...
  test('includes inline styles so the link is offscreen until focused', () => {
    ...
    expect(out).toContain('id="laurel-skip-link-style"');
    expect(out).toMatch(/\.laurel-skip-link\s*\{[^}]*position:absolute/);
    expect(out).toMatch(/\.laurel-skip-link:focus\s*\{[^}]*position:fixed/);
  });
  ...
  test('is idempotent — does not double-inject when already present', () => {
    ...
    const occurrences = twice.match(/class="laurel-skip-link/g) ?? [];
  ...
  test('handles uppercase BODY and arbitrary attributes', () => {
    ...
    const out = injectSkipLink(input);
    expect(out).toContain('class="laurel-skip-link');
    expect(out.indexOf('class="laurel-skip-link')).toBeLessThan(out.indexOf('<span>'));
  });
  ...
  test('stamps the configured CSP nonce onto the skip-link <style> tag', () => {
    ...
    expect(out).toMatch(
      /<style id="laurel-skip-link-style" nonce="abc123">[^<]*\.laurel-skip-link\{/,
    );
  });

  test('omits nonce attribute when no CSP nonce is configured', () => {
    const out = injectSkipLink('<body></body>');
    expect(out).toContain('<style id="laurel-skip-link-style">');
