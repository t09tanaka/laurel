...
Smoke-test inputs for every Koenig card type Laurel's `renderMarkdown` is
...
- Catch regressions where a markdown / sanitisation tweak silently strips a
  ...
  rendered HTML.
- Provide stubs for cards Laurel does not natively expand. Those fixtures
...
HTML shapes during `laurel import-ghost`, and `docs/THEME_DEV.md` for how
