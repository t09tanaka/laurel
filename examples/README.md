...
| `fly/fly.toml`             | Fly.io static-only nginx Machine sample at `examples/fly/fly.toml` that serves a pre-built `dist/` directory with the matching `Dockerfile` and `nginx.conf`. Pairs with `docs/deploy/fly.md`. |
