...
| `examples/docker/Dockerfile`, `examples/docker/Dockerfile.multi-stage`, `examples/docker/.dockerignore`, and `examples/docker/nginx.conf` | Slim `nginx:1.27-alpine` runtime image for an already-built `dist/`, plus a multi-stage Bun build + nginx serve variant with a trimmed build context. Both keep pretty URLs and `404.html` handling. Pairs with `docs/deploy/docker.md`. |
