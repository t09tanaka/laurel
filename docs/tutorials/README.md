...
| 2   | [Migrate from Ghost in 10 minutes](./02-migrate-from-ghost.md)    | Your existing Ghost export rendered as a static Laurel site.          |
...
and a recent Git. Laurel is invoked through `bunx laurel` — no global install
is required.

If you get stuck, run `bunx laurel doctor` for a health check, or open an
