...
Migrated Ghost sites often carry years of redirects. Nectar uses one canonical
...
Nectar also loads Ghost-style `content/data/redirects.{yaml,yml,json}`. The
canonical status set is `301`, `302`, `307`, and `308`; omitted status defaults
to `301`.

## Target matrix

| Target | Nectar setting | Emitted artifact | Redirect syntax Nectar emits | Host consumes it? | Notes |
| --- | --- | --- | --- | --- | --- |
...
| Vercel | `[deploy.vercel].enabled = true` | `dist/vercel.json` | `{ "source": "/old", "destination": "/new", "statusCode": 301 }` in `redirects[]` | Yes | Nectar translates `*` to Vercel path-to-regexp `(.*)`. Vercel applies redirects even when a static file exists at `source`. |
...
| Apache HTTPD | `[deploy.apache].enabled = true` | `dist/.htaccess` | `RewriteRule ^old$ /new [R=301,L]` | Yes, when `.htaccess` is enabled | Requires `AllowOverride FileInfo` and `mod_rewrite`. Nectar also emits cache/header rules into the same file. |
| nginx | `[deploy.nginx].enabled = true` | `dist/.nectar/nginx.conf` | `location = /old { return 301 /new; }` | Yes, after operator includes the file | nginx does not read files from the publish root automatically. Include the generated server block from the main nginx config. |
| Caddy | `[deploy.caddy].enabled = true` | `dist/.nectar/Caddyfile` | `@redirect_0 path /old` plus `redir @redirect_0 /new 301` | Yes, after operator imports or copies the file | Caddy has no `_redirects` convention. Treat the file as operator config, not public content. |
...
| Cloudflare Workers Static Assets | `[deploy.cloudflare_workers].enabled = true` | `dist/_routes-manifest.json` | `{ "source": "/old", "destination": "/new", "status": 301 }` in `redirects[]` | Only with a Worker that reads the manifest | Workers Static Assets does not consume `_redirects`; use Nectar's reference Worker or equivalent code. |
...
| Render Static Sites | none | none | Not emitted | No | Render does not consume Nectar's `_redirects` as a routing contract. Configure redirects in Render until a native emitter exists. |
| DigitalOcean App Platform | none | none | Not emitted | No | App Platform serves `dist/` but Nectar does not emit App Platform redirect config. Use provider settings or a fronting proxy. |
| Bunny.net | none | none | Not emitted | No | Bunny Storage/Pull Zones do not consume Nectar redirect files. Use Edge Rules or a fronting layer. |
...
If `[components.redirects].emit_html = true`, Nectar also writes static HTML
...
- Keep `redirects.yaml` as the source of truth. Do not maintain separate
  ...
  needs behavior Nectar cannot model.
