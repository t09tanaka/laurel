...
Use this target when Vercel should build Nectar, publish `dist/`, and provide
preview and production deployments.

## Recipe

1. Set `site.url`; Vercel preview build URLs can override it for previews.
2. Enable `[deploy.vercel]` so Nectar emits `vercel.json`.
3. Run `bunx nectar build` and confirm `dist/vercel.json`.
4. In Vercel, use the `Other` preset, build command `bunx nectar build`, and
