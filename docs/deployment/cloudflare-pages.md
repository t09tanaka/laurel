...
Use this target when you want Cloudflare Pages to build Nectar from Git and
serve `dist/` on Cloudflare's edge network.

## Recipe

1. Set `site.url` to the production Pages or custom-domain URL.
2. Enable `[deploy.cloudflare_pages]` so Nectar emits Pages files.
3. Run `bunx nectar build` and confirm `dist/_headers` and
