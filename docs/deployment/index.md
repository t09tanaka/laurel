...
Ghost. Each page gives the minimum path to publish Laurel's static `dist/`
output on one host, then links back to the maintained long-form guide under
[`docs/deploy/`](../deploy/).

For every target:

1. Set `site.url` in `laurel.toml` to the final public URL.
2. If the site is not served from `/`, set `build.base_path` before building.
3. Run `bunx laurel build` locally once and inspect `dist/`.
...
| Caddy | You self-host and want generated Caddy config from Laurel. | [caddy.md](./caddy.md) |
...
| nginx | You self-host and want Laurel-generated nginx config. | [nginx.md](./nginx.md) |
...
If browser clients fetch Laurel's emitted Content API from another origin,
