...
`style-src`. **Hashes and nonces are not viable on a static site** — there is
no per-request renderer to emit a fresh nonce, and the inline JSON-LD changes
with every post, which would require regenerating CSP hashes on every build.
If you need a stricter policy, the realistic path is to move all inline
scripts out of templates and remove the `'unsafe-inline'` allowance.
