...
Nectar Admin treats repository files and CLI commands as the source of truth. Settings should expose the current state, safe edit surfaces, and copyable CLI next steps without turning the browser into a deployment or migration runner.
...
- Advanced and dangerous settings are grouped instead of scattered. `build.allow_code_injection`, plugin auto-detect, deploy, import/export, diagnostics, and cache cleanup should not be presented as casual one-click actions.
...
| `nectar import-*` / `nectar export` | Import/export | Discovery and examples only in the first pass |
