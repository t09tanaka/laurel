...
- `{{t "key" [name=value …]}}` — Translation lookup against the theme's `locales/<locale>.json`; string values support interpolation, interpolated values are treated as text with tags stripped, numeric/boolean values are stringified, and present empty-string values render as empty strings.
