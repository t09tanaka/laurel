# Deploying Laurel to Firebase Hosting

Firebase Hosting serves Laurel's already-built `dist/` directory as static
files. Laurel can emit a Firebase Hosting `firebase.json` into the build output
...
native Hosting config. Laurel does not currently ship a `laurel deploy firebase`
...
3. Enable the Firebase emitter in `laurel.toml`:
   ...
   `laurel build` writes `dist/firebase.json` with `hosting.public = "."`.
   ...
   `rewrites: []` by default because Laurel emits real HTML files and
...
5. Deploy the static output:
   ...
   FirebaseExtended/action-hosting-deploy reads Laurel's generated
   `dist/firebase.json`.

## Redirects

Laurel's generic redirects component can emit `dist/_redirects`, but Firebase
Hosting does not read Netlify / Cloudflare Pages `_redirects` files. When
`[deploy.firebase].enabled = true`, Laurel also folds the canonical redirect
...
first-match. Laurel preserves the same collapsed first-match order used by the
other deploy emitters.

## Headers and caching

Firebase reads custom response headers from the `headers` array in
`firebase.json`. With `[deploy.firebase].enabled = true`, Laurel translates
...
Laurel's canonical page shape is a trailing-slash URL backed by a directory
index file:
...
Laurel maps `build.trailing_slash = "always"` to `"trailingSlash": true` and
...
`laurel.toml` and leave `base_path` as `/`:
...
Laurel build behind another Firebase route or CDN path such as `/blog/`, set
`[build].base_path` to that path, including leading and trailing slashes, and
update `[site].url` to the final public URL.

## Troubleshooting

- **Deploy says `dist` does not exist:** run `bunx laurel build` before
...
- **Every route shows the home page:** remove any catch-all rewrite to
  `/index.html`; Laurel is not an SPA.
