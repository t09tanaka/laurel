# Deploying Laurel to Vercel

Vercel can build a Laurel site from Git or receive a pre-built `dist/`
directory from CI. Laurel has two Vercel-specific pieces:

- `[deploy.vercel]` controls build output for Vercel: a generated
  `vercel.json` at the publish root.
- `laurel deploy vercel` shells out to the Vercel CLI for manual or CI-driven
...
own the build, preview, and production promotion flow. Use `laurel deploy
vercel` for manual or custom CI uploads of an already-built `dist/`.

## Quickstart: Vercel builds from Git

1. In `laurel.toml`, enable the Vercel output file:
   ...
   This makes `laurel build` write `dist/vercel.json`. The file folds
...
Laurel is not a Next.js app. It emits a static `dist/` directory directly, so
...
to `Other`, the Build Command set to `bunx laurel build`, and the Output
Directory set to `dist`.

## 404 pages

Laurel always emits `dist/404.html`. If the active theme provides
`error-404.hbs` or `error.hbs`, that template renders the file; otherwise
Laurel writes its built-in branded 404 page.
...
serving Laurel's `dist/404.html` body. No extra rewrite or `routes` entry is
...
`routes` if you intentionally rename the not-found file, which Laurel does
not do by default.

## Clean URLs and trailing slashes

Laurel route pages are directory-index files by default, such as
...
If you opt into `build.trailing_slash = "never"`, Laurel still emits
...
With `[deploy.vercel].enabled = true`, Laurel writes those rules to
`dist/vercel.json`:
...
`Referrer-Policy` by default. Laurel translates glob `*` patterns to Vercel's
...
## CLI deploys with `laurel deploy vercel`
...
The command runs `laurel build` first, checks that
`dist/.laurel-manifest.json` exists, then executes:
...
```sh
bunx laurel deploy vercel --dry-run
```
...
packages Laurel's static `dist/` output during `vercel build`; there is no
...
environment, runs `vercel build` so the Vercel CLI turns `bunx laurel build`
...
This workflow is different from `laurel deploy vercel`: the sample delegates
the build packaging to Vercel CLI's prebuilt pipeline, while `laurel deploy
vercel --build` runs `laurel build` directly and then calls `vercel deploy
dist --prod`. Prefer the sample when you want GitHub Actions parity with
Vercel preview/production environments; prefer `laurel deploy vercel` for a
small manual command or bespoke CI script.

## Preview deploys

On Git-connected preview and branch builds, Vercel provides `VERCEL_URL`.
Laurel uses it automatically as the `site.url` fallback when
`LAUREL_SITE_URL` is unset. Vercel usually provides this value without a
scheme, so Laurel treats it as HTTPS before building canonical, sitemap, RSS,
...
`@site.build.commit_sha`. Explicit `LAUREL_BUILD_METADATA_*` vars and short
aliases such as `LAUREL_COMMIT_SHA` override these provider values.

The URL precedence remains:

```text
--base-url > LAUREL_BUILD_BASE_URL > LAUREL_SITE_URL > VERCEL_URL > site.url
```

Use `--base-url` when you need to force a different host:

```sh
bunx laurel build --base-url https://your-preview.vercel.app
```
...
- **No `vercel.json` in the deploy:** confirm `[deploy.vercel].enabled = true`
  ...
  `dist/vercel.json`.
- **`laurel deploy vercel` opens an interactive login:** set `VERCEL_TOKEN` in
  the environment.
- **The CLI deploy targets the wrong project:** run `vercel link`, or set
  `[deploy.vercel].project` / pass `--project-name` so Laurel forwards
  `--scope`.
