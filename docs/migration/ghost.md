# Migrating from Ghost to Laurel

This is the long-form guide for moving a real blog off Ghost and onto Laurel.
...
> shape of each Ghost or Laurel screen and which control you are about to
...
[`docs/MEMBERS.md`](../MEMBERS.md) before you migrate anything**. Laurel is a
...
- **The Ghost Members database itself.** Laurel does not store emails,
...
- **Server-side paywalls.** A post with `visibility: paid` is truncated to a
  ...
  "did this person pay" — that is a server query Laurel does not perform.
  Wiring a JS-side reveal after auth is your provider's job, not Laurel's.
- **`{{#access}}` content gating.** The `{{access}}` helper always returns
  ...
  [`docs/MEMBERS.md` § 2](../MEMBERS.md#2-what-the-laurel-members-surface-actually-exposes).
...
- **Newsletter email sending.** Ghost's per-post email send pipeline does not
  exist in Laurel. Schedule sends from your provider's dashboard instead.
...
that Laurel is the wrong tool, and you should keep running Ghost (or a
Ghost-compatible backend) for the dynamic surface and use Laurel only for the
...
```
my-blog/
├── laurel.toml                 # site config (replaces Ghost's admin settings)
...
No server. No database. `laurel build` regenerates the whole site from the
...
- **[Bun](https://bun.sh) >= 1.3** on your local machine. (`bun --version`).
  Laurel's CLI is invoked through `bunx laurel` or `bun run`.
...
- **Your Ghost theme**, either as the official Ghost Source theme (vendored in
  the Laurel repo at `example/themes/source/`) or a `.zip` you downloaded from
  ...
  coverage matrix.
- **A new empty directory** for the Laurel project. Don't try to migrate
  in-place over a running Ghost install.

If you do not yet have a Laurel project skeleton, scaffold one:
...
This drops a `laurel.toml`, a starter `themes/source/` link, and a `content/`
tree. You will overwrite most of it during the import, but the layout matters
because `laurel import-ghost` writes into `content/`.
...
Ghost-side settings. Laurel reads the content records from this file; it does
not automatically convert Ghost settings into `laurel.toml`.
...
will pass it to `laurel import-ghost` in the next step.

---

## Step 2 — Run `laurel import-ghost`

From inside your new Laurel project directory:

```bash
bunx laurel import-ghost \
...
2. Converts each post's HTML body to Markdown via
   ...
   and build, Laurel promotes those images back to `kg-image-card` figures for
...
![Terminal output for laurel import-ghost](../migration-from-ghost/02-import-cli.svg)
...
- **Members, subscribers, paid plans.** Laurel is static; there is no
  members database.
- **Ghost settings as automatic config.** Ghost's site title, navigation,
  ...
  copied manually into `laurel.toml` where Laurel has an equivalent.
...
- **Snippets and custom fields outside Laurel frontmatter.** Editor snippets,
...
- **Ghost Admin integrations.** The integrations directory exposed through
  ...
  Ghost Admin concept, not a static-site feature. Laurel does not import or
...
## Step 4 — Edit `laurel.toml`

Laurel's config file replaces almost everything you used to configure in the
...
`laurel init` already wrote a `laurel.toml` skeleton. Open it and fill in
your real values. The shape mirrors the example project's
[`example/laurel.toml`](../../example/laurel.toml); the most important sections
...
> Ghost admin's *Design → Customize* panel. In Laurel they live under
> `[theme.custom]` in `laurel.toml`. Read your theme's `package.json` to see
> which keys it expects. The Source theme example in
> `example/laurel.toml` covers every Source-supported key.

If your theme uses translations (Source ships a `locales/` directory), set
`[site].locale` to match (`en`, `de`, etc.) and Laurel's `{{t}}` helper will
pick the right strings.

### Navigation

Ghost stores primary/secondary navigation in its database. Laurel reads it
from `laurel.toml`:
...
```bash
cp -R /path/to/laurel/example/themes/source themes/source
```
...
Then set `[theme].name = "<your-theme>"` in `laurel.toml`.
...
If your theme references Ghost helpers that Laurel does not yet implement,
...
![Terminal output for laurel build and laurel serve](../migration-from-ghost/04-laurel-build.svg)

`laurel build` writes the static site into `dist/`. `laurel serve` then
...
- **Missing posts.** Posts with `status: scheduled` are excluded from the
  ...
  time — Laurel treats a scheduled date strictly). Posts with
  `visibility: members` or `paid` are imported but Laurel has no membership
...
- **Theme helper errors.** `bunx laurel check --strict` validates the theme
...
3. If hosting under `https://<user>.github.io/<repo>/` from GitHub Actions,
   run the build with `GITHUB_PAGES=true`; Laurel derives
...
`bunx laurel build` and the output directory to `dist`. For Bun version
pinning, use a `vercel.json`:

```json
{
  "buildCommand": "bunx laurel build",
...
- **Set up redirects.** If Ghost served some content at non-default paths
  ...
  redirect file. Laurel uses one URL shape: `/<post-slug>/`, `/<page-slug>/`,
...
- **Decide on members/paid content.** If you ran a paid newsletter on
  Ghost, Laurel does not replace that piece. You can leave those posts
...
- **Check search**, if your theme expects Ghost's `data-ghost-search` buttons.
  Laurel's default JSON search component injects a static modal shim for those
  buttons, and Pagefind engines route them to Pagefind UI. Laurel does not
...
HTML-in-Markdown (Laurel renders Markdown via marked, which passes HTML
...
contexts evaluate to empty values via the Laurel context proxy, so
...
Laurel has no comments backend. Wire up Disqus / Cusdis / utterances as an
optional component, or remove the `{{comments}}` block from your theme.

**"`laurel build` succeeds but a post is missing from the index."** Check
...
- [`docs/GHOST_COMPATIBILITY.md`](../GHOST_COMPATIBILITY.md) — exactly which
  Ghost helpers and context fields Laurel implements, and what is stubbed
  or out of scope.
- [`example/`](../../example/) — a working Laurel project you can `bun
