...
For the supported Ghost Admin export -> Laurel Markdown import path, see
[`MIGRATION.md`](./MIGRATION.md). It lists what `laurel import-ghost` imports
...
- `{{partials}}` is a no-op registration helper. Ghost used it as a directive
  in older themes, but Laurel pre-registers discovered partials at engine
  startup, so rendering an empty string is the compatible static behaviour.
- Laurel-only boolean helpers (`{{or}}`, `{{and}}`, `{{not}}`, `{{eq}}`) are
...
- Themes with `members/signin.hbs`, `members/signup.hbs`,
  ...
  routes. These are presentation routes only; Laurel still does not provide a
...
| `description`    | `laurel.toml [site].description` |
| `url`            | `laurel.toml [site].url` |
| `cdn_url`        | `laurel.toml [site].cdn_url`; used by `{{img_url ... absolute=true}}` for `/content/images/` paths only. Canonical page URLs still use `site.url`. |
| `logo`           | `laurel.toml [site].logo` |
| `icon`           | `laurel.toml [site].icon` |
| `cover_image`    | `laurel.toml [site].cover_image` |
| `lang` / `locale`| `laurel.toml [site].locale` |
| `timezone`       | `laurel.toml [site].timezone` |
| `accent_color`   | `laurel.toml [site].accent_color` |
| `navigation`     | `laurel.toml [[navigation]]` |
| `secondary_navigation` | `laurel.toml [[secondary_navigation]]` |
| `members_enabled` | Always `false`. Laurel has no members backend; this stable default makes Source's sidebar/header/footer/CTA `{{#if @site.members_enabled}}` branches collapse to the public path. |
| `paid_members_enabled` | Always `false`. Same rationale as `members_enabled`; gates the paid-only badge in `post-list.hbs`. |
| `member_count` | Empty by default. Set `[components.portal].member_count` to surface a manually maintained static total for `{{member_count}}` / `@site.member_count`; Laurel never infers live member totals. |
| `portal_button`, `portal_button_icon`, `portal_button_signup_text`, `portal_button_style`, `portal_name`, `portal_plans`, `portal_signup_checkbox_required`, `portal_signup_terms_html`, `signup_url` | From `laurel.toml [site.portal]`. These mirror Ghost Portal settings for themes that probe `@site.portal_*` fields, but Laurel still does not authenticate members by itself. |
...
overrides applied from `laurel.toml [theme.custom]`.
...
This matrix tracks the per-card contract for `laurel import-ghost` and the
Markdown renderer. "Migrates" means a Ghost export can retain enough structure
in `content/posts/*.md` to avoid data loss. "Renders" means a static Laurel
...
| Gallery | Yes | Yes | Preserves the `kg-gallery-container` / row / image shape, intrinsic image dimensions, and width modifier classes; Laurel does not inject Ghost's legacy gallery bootstrap script. |
...
| Signup | Partial | Partial | The `kg-signup-card` wrapper, layout classes, disclaimer, and form hooks can survive for portal/member plugins; Laurel ships static card CSS but no Ghost members backend. |
...
Laurel's Content API exposes `post.html` and `page.html` from the same
Markdown renderer that feeds `{{content}}`. It is Ghost-shaped, but it is not
a byte-for-byte Ghost serializer.

The current contract is:

- Imported or shortcode-authored Koenig cards that Laurel understands render
...
- Members-only `email` / `email-cta` cards are dropped during import and
  ...
  are derived, because Laurel has no authenticated newsletter-only renderer.
  ...
  Paywall markers are converted into Laurel's static public/preview behaviour
  (`feed_html` plus configured visibility policy). Laurel does not emit Ghost's
...
- Raw Ghost-compatible card scaffolds that survive sanitisation, such as
  ...
  through for theme/plugin hydration, but Laurel does not guarantee every
...
- `Post.frontmatter` in Ghost is the raw structured Mobiledoc/Lexical/card deck.
  Laurel does not expose a synthetic `post.frontmatter` field today because a
  safe, useful value requires a content-model contract rather than a lightweight
  render-helper stub.

## AMP and Card Runtime Boundaries

When a theme ships `amp.hbs`, Laurel emits a non-indexable AMP route for every
...
Laurel still does not generate AMP-specific Koenig card markup. Card output
...
implement a Laurel-specific adapter.
...
serializer unless Laurel gains a first-class Lexical/Koenig render path.

## Content API static compatibility

Laurel emits static JSON for Ghost Content API consumers, but it does not run a
...
- Laurel emits `.well-known/ghost.json` for service discovery, but not a
...
- AMP routes, Image API resize URLs, `GET /oembed/`, Admin API webhooks, and a
  separate `@laurel/content-api-types` package are not implemented.

### `error`

The root `error` context is only populated for Laurel's static `/404.html`
...
(`{{{error.message}}}`) are safe in Laurel: with no failed POST happening at
static render time, the path resolves to an empty string and does not throw.
Laurel does not implement a runtime subscribe POST error lifecycle.

### Drafts and private publications

Draft posts/pages are excluded from normal builds at content-load time. Preview
builds can opt in with `laurel build --include-drafts` / `LAUREL_DRAFTS=1`, and
...
input for themes. Laurel does not enforce authentication or render a real
...
header files. If the route URL already has a literal file extension, Laurel
keeps that path.
...
array, matching the rest of Laurel's list route contract. Laurel does not add a
...
Ghost sets root `is_popup` while rendering the subscribe iframe popup. Laurel
...
classes like `.popup` are never added by Laurel.
...
`laurel import-ghost` previously preserved that inner HTML byte-for-byte.
...
Ghost exports rich embeds as `.kg-embed-card` figures. During import, Laurel
...
Laurel renders static iframe embeds only for providers whose public embed URL
...
| Vimeo | `{{< embed url="https://vimeo.com/..." provider="vimeo" />}}` | `https://player.vimeo.com/video/...` iframe | No vendor script is injected by Laurel. Vimeo may still process reader IP/user-agent data when the iframe loads. |
| Spotify | `{{< embed url="https://open.spotify.com/{type}/{id}" provider="spotify" />}}` | `https://open.spotify.com/embed/{type}/{id}` iframe | No vendor script is injected by Laurel. Spotify receives a third-party iframe request when loaded. |
...
| Loom | `{{< embed url="https://www.loom.com/share/..." provider="loom" />}}` | Bookmark-style fallback link | No Loom player script is injected by Laurel. |
...
the provider rather than Laurel. For those providers, Laurel keeps a
...
`shared-theme-assets` bundle, while Laurel only renders static files and
...
Laurel builds and rely on the static markup instead.

### Audio cards

Ghost's Koenig audio card uses `kg-audio-*` markup plus shared runtime
JavaScript for the custom play button, seek slider, and live timestamps. Laurel
does not vendor Ghost's full upstream runtime in static output.

For Source, Laurel vendors stable CSS for the card shell, thumbnail, metadata,
and player hooks, and it relies on the static `<audio controls>` fallback that
Laurel's audio card renderer emits:
...
When a theme opts into `config.card_assets`, Laurel emits its own static
...
Casper-family themes target Koenig card wrapper classes directly. Laurel keeps
...
correctly. Laurel's renderer deliberately emits the card scaffolds only; it
...
shortcodes in Markdown. Laurel stores them as standard Markdown images so the
...
Source and Laurel's shared card CSS style `.kg-code-card`, the nested `pre`,
`figcaption`, and `.kg-code-card-with-line-numbers pre`. Laurel does not
...
Laurel does not inject Ghost Admin or shared-theme-assets JavaScript for
bookmark link tracking. A theme or analytics plugin that needs click tracking
must attach its own listener to `.kg-bookmark-container`; Laurel only guarantees
...
can still target them. Laurel does not currently build those cards from
...
When `config.card_assets` is enabled, Laurel also emits a static signup-card
...
included in the site. Without that vendored runtime, Laurel still renders the
...
theme asset, replace the hook with a Laurel-managed optional component, or
remove the control from the template.

### External script URLs in theme layouts

Laurel leaves theme-authored external `<script>` URLs and integrity metadata
...
leave untouched: Laurel should not rewrite the CDN URL, substitute a newer
...
theme so the script is vendored locally or updated deliberately. Laurel's build
...
when `[deploy.headers].security.content_security_policy` is configured, Laurel
...
- Runtime subscribe / Portal error context (`error.message`) — only Ghost's
  live runtime populates this after failed POSTs. Laurel leaves `error` unset
...
- The Ghost Content API (`{{#get}}` against remote endpoints) — we resolve
  ...
  `/content/*` JSON cross-origin when the host applies Laurel's generated
...
- Ghost-only HTML transforms (responsive image srcsets via Ghost's image
  ...
  later.
- Ghost's `/search/` endpoint and bundled `sodo-search` runtime — Laurel
  ...
  these consumers.
- Ghost's `shared-theme-assets` runtime — not bundled by Laurel. Theme
...
- Ghost Admin's integrations directory (`/ghost/api/integrations`, including
  Zapier, Slack, and similar app listings) — out of scope. Laurel has no Admin
  ...
  deploy provider's integration/webhook settings around `laurel build`.
