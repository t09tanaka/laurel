...
// unconditionally is safe: it adds < 1 KiB to dist/ and lets a single nectar
// build target Azure without operator intervention.
//
// We intentionally keep the emitted payload minimal: a `navigationFallback`
// that points 404s at `/404.html` (matching nectar's `emitDefault404`) and a
...
export function buildStaticWebAppConfig(): AzureStaticWebAppConfig {
  return {
    navigationFallback: {
      ...
      // 404 page rather than rewriting to `/index.html`. `nectar build` emits
