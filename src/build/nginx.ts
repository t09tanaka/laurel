...
//
// The emitted file lands at `<outputDir>/.nectar/nginx.conf` rather than the
// publish root because nginx never serves `.conf` files to clients — keeping
// it under `.nectar/` makes the artifact obviously a build by-product and
...
interface BuildNginxOptions {
  ...
  // their `dist/` path. Defaults to `/var/www/nectar` so the emitted file is
...
export function buildNginxServerBlock(opts: BuildNginxOptions): string {
  const root = opts.root ?? '/var/www/nectar';
  ...
  const lines: string[] = [];
  lines.push('# Generated by Nectar from redirects.yaml + deploy.headers.');
  lines.push('# Include this server block from the main nginx config:');
  lines.push(`#   include ${root}/.nectar/nginx.conf;`);
  lines.push('server {');
...
export async function emitNginxConf(opts: {
  ...
  const targetDir = join(opts.outputDir, '.nectar');
