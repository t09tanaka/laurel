...
const DEFAULT_POLICY_NAME = 'nectar-generated-response-headers';
...
export function buildCloudFrontResponseHeadersPolicy(
  ...
  const policy: CloudFrontResponseHeadersPolicyConfig = {
    ...
    Comment:
      'Generated by Nectar from [deploy.headers].security. Attach to CloudFront cache behaviors; keep cache_rules in S3 metadata or CloudFront cache policies.',
  };
...
export async function emitCloudFrontResponseHeadersPolicy(opts: {
  ...
  const dir = join(opts.outputDir, '.nectar');
  await ensureDir(dir);
  await writeFile(
