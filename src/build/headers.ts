...
export type HeadersConfig = LaurelConfig['deploy']['headers'];
...
// `/content/*` tree is a Laurel-specific build artifact and operators who
...
/**
 * Build the `_headers` body for the Laurel Content API `/content/*` tree.
 *
...
export function buildContentApiHtaccessBody(): string {
  ...
  const lines: string[] = [
    '# Generated by Laurel for Content API CORS.',
    '# Place this file at dist/content/.htaccess and enable AllowOverride FileInfo.',
    '<IfModule mod_rewrite.c>',
    '  RewriteEngine On',
    '  RewriteCond %{ENV:LAUREL_CONTENT_API_CACHE_MATCHED} !1',
  ];
  rules.forEach((rule, index) => {
    if (index > 0) lines.push('  RewriteCond %{ENV:LAUREL_CONTENT_API_CACHE_MATCHED} !1');
    lines.push(
      `  RewriteRule ${rule.pattern} - [E=LAUREL_CONTENT_API_CACHE_${index}:1,E=LAUREL_CONTENT_API_CACHE_MATCHED:1]`,
    );
  });
  lines.push('</IfModule>');
  lines.push('');
  ...
  rules.forEach((rule, index) => {
    lines.push(
      `  Header set Cache-Control "public, max-age=${rule.maxAge}" env=LAUREL_CONTENT_API_CACHE_${index}`,
    );
  });
  lines.push('</IfModule>');
  return `${lines.join('\n')}\n`;
