...
export async function createThemeServeFixture(opts: {
  ...
  const workDir = await mkdtemp(join(tmpdir(), 'laurel-theme-serve-'));
  await writeFixtureContent(workDir);
  const fixtureConfigPath = join(workDir, 'laurel.theme-serve.toml');
...
function resolveThemeRoot(cwd: string, config: LaurelConfig): string {
...
async function writeFixtureContent(workDir: string): Promise<void> {
  ...
  await writeFile(
    join(workDir, 'content/authors/laurel-theme-author.md'),
    [
      '---',
      'name: Theme Author',
      'slug: laurel-theme-author',
      'bio: Minimal author used by `laurel theme serve`.',
      '---',
      '',
    ].join('\n'),
    'utf8',
...
function renderFixtureConfig(sourceConfig: LaurelConfig, themeRoot: string): string {
  const themeDir = dirname(themeRoot);
  const config = {
    site: {
      title: sourceConfig.site.title || 'Theme Fixture',
      description:
        sourceConfig.site.description || 'Small fixture site generated by laurel theme serve',
