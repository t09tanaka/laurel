...
// runs `laurel check` / `laurel build` / `laurel lint`, the underlying loader
...
// the dir layout.
export async function ensureContentDirs(cwd: string, config: LaurelConfig): Promise<string[]> {
  const candidates = [
  ...
  for (const rel of candidates) {
    ...
    try {
      ...
      // throw a LaurelError with the path if it really can't be read.
      logger.warn(
  ...
  if (created.length > 0) {
    ...
    // a fresh checkout. The expected operator response is "edit laurel.toml
    ...
    logger.info(
      `Created missing content directories (${created.length}): ${list}. Add markdown files under them or remove the entries from laurel.toml [content].`,
    );
  }
  return created;
}
