// Live-reload pieces shared by `nectar serve` and `nectar dev`. The two commands
...
export const LIVERELOAD_PATH = '/__nectar_livereload';

// The path requested by `nectar dev` for the standalone client script. The
// task spec lists `/__nectar/livereload.js` as the canonical URL; both that
// path and the inline-served LIVERELOAD_PATH WebSocket endpoint live under
// the `/__nectar` prefix so the surface is grep-able and easy to firewall.
export const LIVERELOAD_SCRIPT_PATH = '/__nectar/livereload.js';

// Standalone client served at LIVERELOAD_SCRIPT_PATH. Browsers fetch this when
// they see `<script src="/__nectar/livereload.js" defer></script>` injected by
// the emit step. Self-guards via window.__nectarLiveReload so repeated injects
...
export const LIVERELOAD_CLIENT_JS = `(function(){
  if (window.__nectarLiveReload) return;
  window.__nectarLiveReload = true;
    ...
    ws.onmessage = function(e) {
      ...
      if (msg.type === 'css') {
        ...
        for (var i = 0; i < links.length; i++) {
          ...
          if (!href) continue;
          var bust = (href.indexOf('?') === -1 ? '?' : '&') + '__nectar=' + Date.now();
...
// Inline (no extra HTTP round-trip) variant used by `nectar serve`'s legacy
...
export const LIVERELOAD_INLINE_SCRIPT = `<script>(function(){if(window.__nectarLiveReload)return;window.__nectarLiveReload=true;var p=location.protocol==='https:'?'wss:':'ws:';function c(){var w;try{w=new WebSocket(p+'//'+location.host+'${LIVERELOAD_PATH}');}catch(e){setTimeout(c,1000);return;}w.onmessage=function(e){var m;try{m=JSON.parse(e.data);}catch(_){m={type:e.data};}if(m.type==='css'){var l=document.querySelectorAll('link[rel=\"stylesheet\"]');for(var i=0;i<l.length;i++){var h=l[i].getAttribute('href');if(!h)continue;l[i].setAttribute('href',h.split('?')[0]+(h.indexOf('?')===-1?'?':'&')+'__nectar='+Date.now());}return;}location.reload();};w.onclose=function(){setTimeout(c,1000);};w.onerror=function(){try{w.close();}catch(_){}};}c();})();</script>`;

// External-script variant used by `nectar dev`. Emitted with `defer` so it
...
//   - 'external' → `nectar dev`: references the script served at
