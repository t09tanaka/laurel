  ...
  .object({
    ...
    build: z
      .object({
        ...
        precompress: z
          .enum(['off', 'brotli', 'gzip', 'both'])
          .default('off')
          .describe(
            'Pre-compress text outputs (`.html`, `.css`, `.js`, `.json`, `.svg`, `.xml`, `.txt`, `.map`) into `<file>.br` (Brotli quality 11) and/or `<file>.gz` (Gzip level 9) siblings. Static hosts that support `brotli_static` / `gzip_static` (Cloudflare Pages, Netlify, nginx) serve the precompressed copy directly when `Accept-Encoding` matches, skipping per-request compression. Choose the format set: `off` (default) skips precompression; `brotli` emits only `.br`; `gzip` emits only `.gz` (use when the host serves `gzip_static` but lacks Brotli, e.g. stock nginx without `ngx_brotli`); `both` emits the full pair for the widest `Accept-Encoding` coverage. Off by default because Brotli q=11 adds noticeable build time on large sites; enable for production builds where transfer size matters more than rebuild latency. Files below 256 bytes are skipped (envelope overhead beats savings) and already-encoded outputs (`.br` / `.gz`) are excluded from a rerun.',
          ),
        csp_nonce: z
          .string()
          .regex(
    ...
    performance: z
      .object({
        preload_lcp_image: z
          ...
          .default(true)
          .describe(
            'Inject `<link rel="preload" as="image" fetchpriority="high" href="…">` for the route\'s LCP image, then align it with the final rendered `<img fetchpriority="high">` candidate (its `src`, `srcset`/`sizes`, or `<picture>` source). On post / page routes with a `feature_image` the preload is emitted in `{{ghost_head}}`; on list / archive / tag / home feeds (and posts whose LCP is a promoted first content image rather than a cover) it is injected during HTML post-processing from the first `fetchpriority="high"` `<img>`, carrying that image\'s own `srcset`/`sizes` so the browser preloads the exact candidate it renders. This starts the LCP image from the HTML preload scan without preloading a stale source. Disable when a custom theme already emits its own LCP preload to avoid double-fetching.',
          ),
        ...
        critical_css: z
          .object({
            ...
            max_inline_kb: z
              ...
              .default(50)
              .describe(
                'Skip inlining (and leave the stylesheet render-blocking) when a route\'s extracted critical CSS would exceed this many kilobytes. Guards against pages whose "used CSS" is nearly the whole sheet, where inlining would bloat the HTML more than it helps.',
              ),
            safelist: z
              .array(z.string())
              .default([])
              .describe(
                'Selector substrings/patterns (compiled as regular expressions) whose rules are always inlined regardless of whether the route HTML appears to use them. Use for classes a client script adds after load that must be styled before the async sheet arrives.',
              ),
          })
          .strict()
          .default({})
        ...
        images: z
          .object({
            ...
            srcset_max_ratio: z
              ...
              .optional()
              .describe(
                'When set, fill gaps in responsive `srcset`s so no two adjacent width candidates differ by more than this ratio. Densifies both Laurel-injected body-image srcsets (the default responsive widths) and theme-emitted card/feature srcsets (whose widths are the theme `image_sizes` keys), inserting intermediate `/content/images/size/wXXX/` widths plus their per-format siblings. Closes the common 600w->1000w gap so a browser needing ~700px does not download the 1000w file. Smaller ratio = more sizes = tighter fit but more variant files on disk; Ghost recommends keeping the total number of sizes modest, so 1.5 is a reasonable starting point. Requires `resize` + sharp; widths that would upscale the source are skipped. Unset (default) leaves srcsets as-is.',
              ),
            webp_quality: z
              .number()
              .int()
